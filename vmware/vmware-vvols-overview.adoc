...
Bien que VAAI propose diverses opérations déportées vers le stockage, certaines lacunes sont comblées par le fournisseur VASA. SAN VAAI ne peut pas déporter les snapshots gérés par VMware vers le système de stockage. NFS VAAI peut déporter les snapshots gérés par les machines virtuelles, mais des limitations s'appliquent à une machine virtuelle disposant de snapshots natifs du stockage. Puisque vVols utilise des LUN, des espaces de noms ou des fichiers individuels pour les disques des machines virtuelles, ONTAP peut cloner rapidement et efficacement les fichiers ou LUN afin de créer des snapshots granulaires par machine virtuelle, qui ne nécessitent plus de fichiers delta. NFS VAAI ne prend pas non plus en charge le déchargement des opérations de clonage pour les migrations Storage vMotion à chaud (machines sous tension). La machine virtuelle doit être hors tension pour permettre le déchargement de la migration lors de l'utilisation de VAAI avec des datastores NFS traditionnels. Le fournisseur VASA dans les outils ONTAP permet des clones quasi instantanés et efficaces en termes de stockage pour les migrations à chaud et à froid, et prend également en charge des copies quasi instantanées pour les migrations inter-volumes de vVols. Grâce à ces gains significatifs en matière d'efficacité de stockage, vous pourrez tirer pleinement parti des charges de travail vVols dans le cadre du programme  https://www.netapp.com/pdf.html?item=/media/79014-ng-937-Efficiency-Guarantee-Customer-Flyer.pdf["Garantie d'efficacité"]. De même, si les clones inter-volumes utilisant VAAI ne répondent pas à vos besoins, vous pourrez probablement résoudre votre défi métier grâce aux améliorations apportées à l'expérience de copie avec vVols.
