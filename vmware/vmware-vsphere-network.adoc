...
Les administrateurs de virtualisation privilégient parfois la simplification de leurs infrastructures en supprimant les commutateurs réseau de la configuration. Cela permet de créer une solution plus autonome, avec moins de composants et un encombrement réduit dans la baie. Il existe des restrictions importantes qui dépendent du protocole utilisé.



=== Fibre Channel

À partir d'ONTAP 9.19.1, la connexion FC directe est prise en charge avec les SAN FC traditionnels (SCSI) et NVMe/FC avec certains adaptateurs FC pour systèmes de stockage. Les HBA établiront une connexion FC point à point standard.

Avant ONTAP 9.19.1, les systèmes de stockage ONTAP nécessitent NPIV, ce qui signifie que le système de stockage doit être connecté à un commutateur FC qui prend également en charge NPIV.
