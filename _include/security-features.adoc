...
Depuis la version ONTAP 9.12.1, les snapshots ne sont plus seulement en lecture seule, ils peuvent également être protégés contre toute suppression accidentelle ou intentionnelle. Cette fonctionnalité est appelée snapshots inviolables. Une période de conservation peut être définie et appliquée via la politique de snapshots. Les snapshots ainsi créés ne peuvent pas être supprimés avant d'avoir atteint leur date d'expiration. Il n'existe aucune dérogation administrative ou via le centre de support.
