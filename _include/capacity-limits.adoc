= Agrégats SSD AFF et zones de disponibilité de stockage ASA
:allow-uri-read: 


L'espace libre est défini comme tout espace non utilisé pour des données réelles et comprend l'espace non alloué sur l'agrégat lui-même ainsi que l'espace inutilisé au sein des volumes qui le composent. Le provisionnement fin doit également être pris en compte. Par exemple, un volume AFF peut contenir un LUN de 1 To dont seulement 50 % est utilisé par des données réelles. Dans un environnement à provisionnement fin, cela apparaîtrait correctement comme consommant 500 Go d'espace. Cependant, dans un environnement à provisionnement complet, la totalité de la capacité de 1 To semble être utilisée. Les 500 Go d'espace non alloué sont masqués. Cet espace n'est pas utilisé par des données réelles et doit donc être inclus dans le calcul de l'espace libre total.
