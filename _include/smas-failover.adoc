...
Les schémas illustrent ce qui se passe lors des opérations normales, de basculement et de retour arrière. Par souci de simplification, ils représentent un LUN répliqué. Dans une configuration SnapMirror active sync réelle, la réplication est basée sur des volumes AFF, chaque volume contenant un ou plusieurs LUN, mais pour simplifier la représentation, la couche volume a été omise.
