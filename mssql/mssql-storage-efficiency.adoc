...
* N'activez pas la déduplication sur les volumes FAS contenant des fichiers de données SQL Server, sauf si le volume contient plusieurs copies des mêmes données, telles que la restauration d'une base de données à partir de sauvegardes sur un seul volume.
