...
Les agrégats constituent le niveau de stockage le plus bas pour les configurations de stockage AFF. Certaines documentations anciennes disponibles sur Internet recommandent de séparer les E/S sur différents ensembles de disques sous-jacents. Cette approche est déconseillée avec ONTAP. NetApp a réalisé divers tests de caractérisation de la charge de travail d’E/S à l’aide d’agrégats partagés et dédiés, avec des fichiers de données et des fichiers journaux de transactions séparés. Les tests démontrent qu’un agrégat de grande taille, comportant davantage de groupes RAID et de disques, optimise et améliore les performances de stockage et simplifie la gestion pour les administrateurs, et ce pour deux raisons :

* Un seul agrégat/SA de grande taille met les capacités d'E/S de tous les disques à la disposition de tous les fichiers.
