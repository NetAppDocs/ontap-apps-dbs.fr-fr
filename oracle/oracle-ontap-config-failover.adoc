...
Lors des opérations de maintenance courantes, une prise de contrôle de stockage ou une opération de basculement MetroCluster doit être transparente, hormis une éventuelle brève interruption de service due à la modification des chemins réseau. La mise en réseau peut être complexe et il est facile de faire des erreurs, c'est pourquoi NetApp recommande fortement de tester minutieusement les opérations de prise de contrôle et de basculement avant de mettre un système de stockage en production. C'est le seul moyen de garantir que tous les chemins réseau sont correctement configurés. Dans un environnement SAN, vérifiez attentivement le résultat de la commande  `sanlun lun show -p` pour vous assurer que tous les chemins principaux et secondaires attendus sont disponibles.
