...
Pour protéger les données contre ce type de situation, ONTAP permet de configurer des volumes, des LUN ou des espaces de noms NVMe pour une protection spéciale contre les défaillances de la NVRAM. Lorsqu'il est déclenché, ce mécanisme de protection fait passer les ressources de stockage dans un état appelé NVFAIL. Cet état provoque des erreurs d'E/S qui entraînent l'arrêt des applications afin qu'elles n'utilisent pas de données obsolètes. Aucune donnée ne devrait être perdue, car toute écriture confirmée devrait être présente sur la baie de stockage.

L'étape suivante consiste généralement pour un administrateur à arrêter complètement les hôtes avant de remettre manuellement en ligne les LUN, les espaces de noms ou les volumes NFS. Bien que ces étapes puissent demander un certain travail, cette approche reste la plus sûre pour garantir l'intégrité des données. Toutes les données ne nécessitent pas cette protection, c'est pourquoi le comportement de NVFAIL peut être configuré de manière granulaire.
