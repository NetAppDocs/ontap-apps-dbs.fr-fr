...
L'exigence principale est l'isolation des fichiers de données dans un ou plusieurs conteneurs dédiés. Cela peut être réalisé de plusieurs façons, notamment en plaçant tous les fichiers de données sur un seul LUN/espace de noms, dans un seul volume AFF NFS, ou dans un groupe de cohérence ONTAP multi-volumes ou multi-LUN/espaces de noms. L'essentiel est que les fichiers de données ne soient contaminés par aucun autre type de fichier. La raison est de s'assurer que la réplication des fichiers de données soit totalement indépendante de la réplication d'autres types de données, tels que les journaux d'archive. Pour plus d'informations sur l'organisation des fichiers et pour des détails importants sur la manière de garantir une organisation de stockage compatible avec les snapshots, consultez link:oracle-dp-online-backup.html#data-layout["Disposition des données"].
