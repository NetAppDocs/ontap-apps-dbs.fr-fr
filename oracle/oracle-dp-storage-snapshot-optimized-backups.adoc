...
La solution la plus simple consiste à isoler les fichiers de données dans des volumes dédiés, des LUN ou des espaces de noms NVMe. Les ressources de stockage ne doivent contenir aucun autre type de fichier. Ceci afin de garantir que les fichiers de données puissent être rapidement restaurés grâce à une opération SnapRestore sans endommager un journal de restauration important, un fichier de contrôle ou un journal d'archivage.
