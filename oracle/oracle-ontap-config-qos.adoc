...
À titre d'exemple, un ancien système AFF A800 HA à deux nœuds peut gérer jusqu'à un million d'IOPS aléatoires avant que la latence ne dépasse une milliseconde. Rares sont les charges de travail uniques qui atteindraient de tels niveaux. Pour exploiter pleinement ce système AFF A800, il faut héberger plusieurs charges de travail, et le faire en toute sécurité tout en assurant la prévisibilité nécessite des contrôles QoS.

Il existe deux types de qualité de service (QoS) dans ONTAP : les IOPS et la bande passante. Les contrôles QoS peuvent être appliqués aux SVM AFF, aux volumes AFF, aux fichiers AFF, ainsi qu’aux LUN et espaces de noms AFF/ASA.
