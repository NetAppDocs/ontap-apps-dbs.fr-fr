...
Prenons l'exemple d'une base de données stockée sur un LUN unique de 250 Go au sein d'un volume de 1 To sur un système AFF. La création d'un instantané entraînerait immédiatement la réservation de 250 Go supplémentaires dans le volume afin de garantir que le volume ne soit jamais saturé pour quelque raison que ce soit. L'utilisation de réserves fractionnaires est généralement inutile, car il est extrêmement improbable que chaque octet du volume de la base de données doive être réécrit. Il n'y a aucune raison de réserver de l'espace pour un événement qui ne se produit jamais. Toutefois, si un client ne peut pas surveiller la consommation d'espace dans un système de stockage et doit être certain que l'espace ne sera jamais saturé, des réserves fractionnaires à 100 % seront nécessaires pour utiliser les instantanés.


TIP: L'abstraction de volume n'est pas présente sur SAN-only l'ASA, et par conséquent cette plateforme n'utilise pas de réserves fractionnaires.
...
Il est toujours possible que la compressibilité d'un ensemble de données change, ce qui entraînerait une augmentation de la consommation d'espace réel. Cette augmentation de la consommation signifie que la compression doit être gérée comme pour les autres formes de provisionnement fin en termes de surveillance et, sur un système AFF, à l'aide de  `volume-autogrow` et  `snapshot-autodelete`.
...
[TIP]
====
*NetApp recommande* les configurations de réserve suivantes avec les plateformes AFF et FAS :
