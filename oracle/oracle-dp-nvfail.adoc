...
[NOTE]
====
* Pour les bases de données Oracle, NVFAIL doit être activé sur tous les LUN hébergeant des composants critiques (fichiers de données, journaux de redo, fichiers de contrôle).
* Sur les systèmes AFF, NVFAIL fonctionne au niveau du volume et s'applique à toutes les données contenues.
* Sur ASA, NVFAIL fonctionne au niveau LUN ou espace de noms.
