...
[role="lead"]
Le placement optimal des LUN de base de données dans les volumes ONTAP sur les systèmes AFF et FAS dépend principalement de la manière dont les différentes fonctionnalités ONTAP seront utilisées.

[]
====
* Cette section s'applique aux systèmes AFF et FAS.
* ASA est une plateforme SAN simplifiée et n'expose pas de couche d'abstraction,
