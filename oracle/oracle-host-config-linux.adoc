...
Une option de montage supplémentaire, appelée  `nosharecache`, est requise lorsque (a) DNFS est activé et (b) un partage NFS source est monté plusieurs fois sur un même serveur (c) avec un montage NFS imbriqué. Cette configuration est principalement rencontrée dans les environnements prenant en charge les applications SAP. Par exemple, un partage NFS unique sur un NetApp système peut avoir un répertoire situé à  `/vol/oracle/base` et un second à  `/vol/oracle/home`. Si  `/vol/oracle/base` est monté à  `/oracle` et  `/vol/oracle/home` est monté à  `/oracle/home`, il en résulte des montages NFS imbriqués provenant de la même source.

Le système d'exploitation peut détecter que  `/oracle` et  `/oracle/home` résident sur le même partage NFS, qui est le même système de fichiers source. Le système d'exploitation utilise alors le même descripteur de périphérique pour accéder aux données. Cela améliore l'utilisation du cache du système d'exploitation et certaines autres opérations, mais cela interfère avec DNFS. Si DNFS doit accéder à un fichier, tel que le  `spfile`, sur  `/oracle/home`, il pourrait tenter par erreur d'utiliser le mauvais chemin vers les données. Le résultat est une opération d'E/S ayant échoué. Dans ces configurations, ajoutez l'option de montage  `nosharecache` à tout système de fichiers NFS qui partage un partage NFS source avec un autre système de fichiers NFS sur cet hôte. Cela force le système d'exploitation Linux à allouer un descripteur de périphérique indépendant pour ce système de fichiers.
